...
//Para TODOS los niños de la solucion, todos deben estar contentos
bool satisface(vector<int> sol, tDatos d) {
    ...
    for (int z = 0; z < sol.size(); z+=2 ) { //Sumo 2 ya que cada niño tendra 2 juguetes
        ...
        if (sum < d.minSat) return false; //Hay algun niño que no esta satisfecho
        nyno++;
    }
    return true;
}
    ...
    for (int i = 0; i < d.numJuguetes; i++) {
        ...
        if (esValida(sol, k, i, d)) { //Es valida
            if (k == (sol.size() - 1)) { //Es solucion
                if (satisface(sol, d)) { //cmprobamos que todos los niños esten satisfechos
                    numSol++;
                    escribirSol(sol);
                }
            }
        ...
        //Aqui se desmarcaria --> sumamos las unidades de ese juguete
        ++d.unidadesJuguetes[i];
    }
    return numSol;
}
