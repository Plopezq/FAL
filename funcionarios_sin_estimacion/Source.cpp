...
void resolver(tDatos &datos, int k, tSol& sol,vector<bool> &marcas, int &tiempoMinimo ) {
    ...
    for (int i = 0; i < datos.num; i++) {
        ...
        if (!marcas[i]) { //es valido, es decir, ese trabajo no lo esta haciendo otro
            ...
            if (k == (sol.sol.size() - 1)) { //si es solucion
                ...
                resolver(datos, k + 1, sol, marcas, tiempoMinimo);
            }
...
bool resuelveCaso() {
    ...
    resolver(datos, 0, sol, marcas, tiempoMinimo);
