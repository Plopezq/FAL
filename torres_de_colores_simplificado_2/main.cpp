...
//Funcion que comprueba que no se colocan dos piezas verdes seguidas
bool esValida(const int k, vector<int>& sol) {
...
void permutaciones(int const altura, int k, vector<int>& sol) {
    for (int i = 0; i < 3; i++) {
        sol[k] = i;
        if (esValida(k, sol)) {//No tiene dos verdes seguidos
